...
> **当前版本：v2.3**（2026-05-11 升级）
> **版本说明**：v2.3 修复了策略入场区间计算缺陷——做空时用 `min(price×1.008, ema20)` 会被远低于当前价的EMA20拉低数千点导致无法触发；做多时同理被布林中轨截断。新逻辑：入场区间锚定在当前价±0.2%~0.5%，止损基于近期高/低点，止盈按2:1/3:1盈亏比计算，不再依赖远离价格的EMA20或布林带。
...
| v2.3 | 2026-05-11 | **修复入场区间计算逻辑**：做空/做多入场区间不再用 EMA20/布林带截断，改为锚定当前价±0.2%~0.5%；止损基于近期高低点；止盈按2:1/3:1盈亏比计算 |
